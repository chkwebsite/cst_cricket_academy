...
import MatchBowlingForm, { getEmptyBowling, normalizeBowlingPayload } from "../MatchBowlingForm";

export default function NewMatchBowlingPage() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (!res.ok) throw new Error(json.message || "Unable to add bowling record.");
                    ...
                    <div className="bg-white rounded-4 shadow-sm p-4 mb-4">
                        <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                            <div>
                                ...
                                <h1 className="h4 mb-2">Add Bowling Record</h1>
                                <p className="text-muted mb-0">Record a bowler's figures for an innings.</p>
                            </div>
                            <button className="btn btn-primary" type="submit" form="matchBowlingForm" disabled={saving || loading}>
                                <Plus size={16} className="me-2" /> {saving ? "Saving..." : "Save Record"}
                            </button>
                        </div>
                    </div>
                </div>
